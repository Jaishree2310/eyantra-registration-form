  ...
  <div class="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow-md">
    ...
    <div v-else-if="apiError" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-6">
      <div class="flex">
        ...
        <div>
          <p>{{ apiError }}</p>
          <button @click="fetchReferenceData" class="mt-2 text-[#E31E24] font-semibold hover:underline">
            Retry
          </button>
        </div>
      </div>
    ...
    <form v-else @submit.prevent="submitForm" class="space-y-6">
      <!-- Name Field -->
      <FormField label="Full Name" :error="v$.name.$errors.length > 0 ? v$.name.$errors[0].$message : ''">
        <input v-model="form.name" type="text" class="form-input" :class="{ 'border-red-500': v$.name.$error }"
          @blur="v$.name.$touch()" />
      </FormField>

      <!-- Email Field -->
      <FormField label="Email ID" :error="v$.email.$errors.length > 0 ? v$.email.$errors[0].$message : ''">
        <input v-model="form.email" type="email" class="form-input" :class="{ 'border-red-500': v$.email.$error }"
          @blur="v$.email.$touch()" />
      </FormField>
      ...
      <FormField label="Contact Number"
        ...
        <input v-model="form.contactNumber" type="tel" class="form-input"
          :class="{ 'border-red-500': v$.contactNumber.$error }" @blur="v$.contactNumber.$touch()" />
      </FormField>

      <!-- Gender Field -->
      <FormField label="Gender" :error="v$.gender.$errors.length > 0 ? v$.gender.$errors[0].$message : ''">
      ...
      <!-- Country Field -->
      <FormField label="Country" :error="v$.country.$errors.length > 0 ? v$.country.$errors[0].$message : ''">
        <select v-model="form.country" class="form-select" :class="{ 'border-red-500': v$.country.$error }"
      ...
      <!-- College Field -->
      <FormField label="College" :error="v$.college.$errors.length > 0 ? v$.college.$errors[0].$message : ''">
        <select v-model="form.college" class="form-select" :class="{ 'border-red-500': v$.college.$error }"
      ...
      <!-- Year Field -->
      <FormField label="Year" :error="v$.year.$errors.length > 0 ? v$.year.$errors[0].$message : ''">
        <select v-model="form.year" class="form-select" :class="{ 'border-red-500': v$.year.$error }"
          @blur="v$.year.$touch()">
      ...
      <!-- Department Field -->
      <FormField label="Domain / Department"
        :error="v$.department.$errors.length > 0 ? v$.department.$errors[0].$message : ''">
        <select v-model="form.department" class="form-select" :class="{ 'border-red-500': v$.department.$error }"
    ...
    <transition name="fade">
      <div v-if="showSuccessModal" class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
        <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full text-center">
          ...
          <p class="mb-6">Thank you for registering for the competition. We'll contact you soon with further details.
          </p>
